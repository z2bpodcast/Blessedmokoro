...
// Auto-scale quote to fill available space
...
function goldBar(ctx: CanvasRenderingContext2D, W: number) {
...
// Brand block — title: Z2B TABLE BANQUET (44px), subtitle: Entrepreneurial Consumer Workshop (24px)
function drawBrand(ctx: CanvasRenderingContext2D, W: number, c1: string, c2: string) {
  ctx.textAlign = "center";
  ctx.fillStyle = c1;
  ctx.font = "bold 44px Georgia,serif";          // +2pt from original 42px
  ctx.fillText("Z2B TABLE BANQUET", W / 2, 88);
  ctx.fillStyle = "rgba(212,175,55,0.35)";
  ctx.fillRect(W / 2 - 230, 100, 460, 1.5);
  ctx.fillStyle = c2;
  ctx.font = "24px Georgia,serif";               // +2pt from original 22px
...
function renderPoster(
  ...
  if (templateId === "royal") {
    ...
    ctx.globalAlpha = 1;
    const g = goldBar(ctx, W); ctx.fillStyle = g; ctx.fillRect(0, 0, W, 6);
    ...
    ctx.fillStyle = bg; ctx.fillRect(0, 0, W, H);
    const g = goldBar(ctx, W); ctx.fillStyle = g; ctx.fillRect(0, 0, W, 5);
...
export default function ECPosterStudio({ builderRef, builderName }: ECPosterStudioProps) {
  ...
  const handleDownload = () => {
    ...
    a.href     = imageUrl;
    a.download = `Z2B-Poster-${selected}-${Date.now()}.png`;
    a.click();
  };
  ...
  // WhatsApp — sends caption text only (browsers cannot auto-attach images)
  const handleWhatsApp = () => {
    window.open(`https://wa.me/?text=${encodeURIComponent(caption)}`, "_blank");
  };

  // Facebook — sends caption + link
  ...
  // TikTok — download poster + copy caption
  const handleTikTok = () => {
    handleDownload();
    handleCopyCaption();
  };
  ...
  const CSS: Record<string, React.CSSProperties> = {
    fab: {
      ...
      letterSpacing: "0.3px", transition: "transform 0.2s",
    },
    backdrop: {
    ...
    hint: {
      ...
      color: "#DDD6FE", lineHeight: 1.6, marginBottom: "14px",
    },
    textarea: {
      width: "100%", background: "rgba(255,255,255,0.05)",
      border: "1.5px solid rgba(255,255,255,0.1)", borderRadius: "10px",
      padding: "12px 14px", color: "#F5F3FF", fontSize: "14px",
    ...
    swatch: (id: string) => ({
      ...
      background: TEMPLATES.find(t => t.id === id)!.preview,
    }),
    previewImg: {
      width: "100%", height: "100%",
      objectFit: "cover" as const, display: "block",
    },
    btn: (color: string, text = "#F5D060") => ({
    ...
    shareBtn: (bg: string, border?: string) => ({
      ...
      cursor: "pointer", fontFamily: "Georgia,serif",
      marginBottom: "10px", transition: "transform 0.15s",
    }),
    captionBox: {
      ...
      maxHeight: "130px", overflowY: "auto" as const,
      marginBottom: "10px",
    },
    note: {
      fontSize: "11px", color: "rgba(255,255,255,0.25)",
      textAlign: "center" as const, lineHeight: 1.5, marginTop: "8px",
    },
    closeBtn: {
  ...
  return (
    <>
      ...
      {/* Backdrop */}
      <div style={CSS.backdrop} onClick={() => { setIsOpen(false); setStep("pick"); }} />
      ...
      <div style={CSS.modal} onClick={e => e.stopPropagation()}>
        ...
        <div style={CSS.header}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            <button style={CSS.closeBtn} onClick={() => { setIsOpen(false); setStep("pick"); }}>×</button>
          </div>
        </div>
        ...
        {step === "pick" && (
          <div style={CSS.body}>
            ...
            <button
              style={{ ...CSS.btn("linear-gradient(135deg,#4C1D95,#7C3AED)"), marginTop: "20px", border: "1.5px solid #D4AF37" }}
          ...
          <div style={CSS.body}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
              ...
              <div>
                ...
                <div style={CSS.hint}>The text auto-scales to fill the poster beautifully — short or long quotes both work.</div>
                <textarea
                  value={quoteText}
                  ...
                  <div style={CSS.swatchGrid}>
                    {TEMPLATES.map(t => (
                      <div
                        ...
                        title={t.name}
                        onClick={() => setSelected(t.id)}
                        style={CSS.swatch(t.id)}
                      />
                    ))}
                  </div>
                </div>
                {/* Ref tag */}
                <div style={{ marginTop: "14px", background: "rgba(212,175,55,0.07)", border: "1px solid rgba(212,175,55,0.18)", borderRadius: "10px", padding: "10px 14px" }}>
                  <div style={{ fontSize: "10px", fontWeight: 700, color: "rgba(212,175,55,0.7)", letterSpacing: "0.5px", marginBottom: "4px" }}>REFERRAL LINK (auto-included on poster)</div>
              ...
              <div>
                ...
                <div style={{ borderRadius: "12px", overflow: "hidden", border: "1.5px solid rgba(212,175,55,0.25)", aspectRatio: "1080/1350", background: template.preview }}>
                  {imageUrl
            ...
            <div style={{ display: "flex", gap: "10px", marginTop: "20px" }}>
              <button onClick={() => setStep("pick")} style={{ padding: "13px 20px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "11px", color: "rgba(255,255,255,0.55)", fontSize: "13px", cursor: "pointer", fontFamily: "Georgia,serif" }}>← Back</button>
              <button
                disabled={quoteText.trim().length < 5}
                onClick={() => setStep("share")}
                style={{ flex: 1, padding: "13px", background: quoteText.trim().length >= 5 ? "linear-gradient(135deg,#4C1D95,#7C3AED)" : "rgba(255,255,255,0.05)", border: "1.5px solid rgba(212,175,55,0.4)", borderRadius: "12px", color: quoteText.trim().length >= 5 ? "#F5D060" : "rgba(255,255,255,0.25)", fontSize: "15px", fontWeight: 700, cursor: quoteText.trim().length >= 5 ? "pointer" : "not-allowed", fontFamily: "Georgia,serif" }}
              >
          ...
          <div style={CSS.body}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
              ...
              <div>
                ...
                <div style={{ borderRadius: "14px", overflow: "hidden", border: "2px solid rgba(212,175,55,0.35)", aspectRatio: "1080/1350" }}>
                  {imageUrl && <img src={imageUrl} alt="poster" style={CSS.previewImg} />}
                </div>
                <button style={CSS.outlineBtn} onClick={handleDownload}>⬇ Download Poster (PNG)</button>
                <button onClick={() => setStep("edit")} style={{ width: "100%", background: "none", border: "none", color: "rgba(167,139,250,0.5)", fontSize: "12px", cursor: "pointer", fontFamily: "Georgia,serif", textDecoration: "underline" }}>← Edit quote or template</button>
              </div>
              ...
              <div>
                ...
                <button style={{ ...CSS.outlineBtn, background: captionCopied ? "rgba(16,185,129,0.12)" : "rgba(139,92,246,0.12)", borderColor: captionCopied ? "rgba(16,185,129,0.35)" : "rgba(139,92,246,0.3)", color: captionCopied ? "#6EE7B7" : "#DDD6FE" }} onClick={handleCopyCaption}>
                  {captionCopied ? "✓ Caption Copied!" : "📋 Copy Caption"}
                </button>

                <label style={{ ...CSS.label, marginTop: "14px" }}>Share</label>

                <button style={CSS.shareBtn("#25D366")} onClick={handleWhatsApp}>
                  ...
                  <span style={{ marginLeft: "auto", fontSize: "11px", opacity: 0.65, fontWeight: 400 }}>attach poster manually</span>
                </button>

                <button style={CSS.shareBtn("#1877F2")} onClick={handleFacebook}>
                  <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.994 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.874 2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                  Share to Facebook
                  <span style={{ marginLeft: "auto", fontSize: "11px", opacity: 0.65, fontWeight: 400 }}>attach poster manually</span>
                </button>

                <button style={CSS.shareBtn("#010101", "#69C9D0")} onClick={handleTikTok}>
                  ...
                  <span style={{ marginLeft: "auto", fontSize: "11px", opacity: 0.65, fontWeight: 400 }}>paste in app</span>
                </button>
