...
// Builder connects Buffer once → posts auto-publish on schedule
//
// HOW IT WORKS:
// 1. Builder signs up free at buffer.com (free: 3 channels, 10 scheduled posts)
// 2. Builder creates a Buffer Access Token (buffer.com/developers/api)
// 3. Pastes token here once — saved encrypted in Supabase
// 4. Content Studio sends posts directly to Buffer queue
// 5. Buffer publishes to Facebook Page + Instagram Business on the scheduled date/time
...
interface BufferChannel {
  id: string
  service: string        // 'facebook' | 'instagram' | 'twitter' | 'linkedin'
...
export default function BufferConnect({ onChannelsLoaded, compact = false }: BufferConnectProps) {
  ...
  const verifyAndSave = async () => {
    ...
    try {
      // Call our API route which calls Buffer — keeps token server-side
      const res = await fetch('/api/buffer/verify', {
        method: 'POST',
      ...
      // Save token + channels to Supabase
      setSaving(true)
      ...
      {status !== 'connected' ? (
        <>
          ...
          <div style={{ background:'rgba(0,186,124,0.06)', border:'1px solid rgba(0,186,124,0.15)', borderRadius:'10px', padding:'14px', marginBottom:'14px' }}>
            <div style={{ fontSize:'11px', fontWeight:700, color:'rgba(0,186,124,0.8)', letterSpacing:'1px', marginBottom:'10px' }}>3 STEPS — ONE TIME SETUP</div>
            {[
              { n:'1', text:'Go to buffer.com → Sign up free (takes 2 min)', link:'https://buffer.com', linkText:'Open Buffer →' },
              { n:'2', text:'Connect your Facebook Page and/or Instagram Business account in Buffer' },
              { n:'3', text:'Go to buffer.com/developers/api → Create Access Token → paste below', link:'https://buffer.com/developers/apps', linkText:'Get Token →' },
            ].map(step => (
          ...
          <button onClick={verifyAndSave} disabled={!token.trim() || loading || saving}
            style={{ width:'100%', padding:'12px', background: token.trim()&&!loading ? 'linear-gradient(135deg,#065F46,#059669)' : 'rgba(255,255,255,0.05)', border:`1px solid ${token.trim()&&!loading?'rgba(16,185,129,0.4)':'rgba(255,255,255,0.08)'}`, borderRadius:'10px', color: token.trim()&&!loading ? '#6EE7B7' : 'rgba(255,255,255,0.25)', fontWeight:700, fontSize:'14px', cursor: token.trim()&&!loading ? 'pointer' : 'not-allowed', fontFamily:'Georgia,serif' }}>
          ...
          {channels.length === 0 && (
            <div style={{ padding:'12px', background:'rgba(251,146,60,0.08)', border:'1px solid rgba(251,146,60,0.2)', borderRadius:'10px', marginBottom:'12px' }}>
              <div style={{ fontSize:'12px', color:'#FB923C', lineHeight:1.6 }}>
                ...
                <br/>Add them at <a href="https://buffer.com/channels" target="_blank" rel="noreferrer" style={{ color:'#FB923C', fontWeight:700 }}>buffer.com/channels</a> then refresh this page.
              </div>
            </div>
          )}

          <button onClick={disconnect} style={{ padding:'8px 18px', background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:'10px', color:'rgba(239,68,68,0.6)', fontSize:'12px', cursor:'pointer', fontFamily:'Georgia,serif' }}>
            Disconnect Buffer
          </button>
        </>
      )}
