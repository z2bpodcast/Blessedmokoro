...
export default function FournityProductCard({ memberReferralCode }: { memberReferralCode?: string }) {
  ...
  if (orderComplete) return (
    <div style={{background:'#12121A',border:'1px solid rgba(201,168,76,0.25)',borderRadius:8,padding:'32px 24px',textAlign:'center' as const,fontFamily:'Inter,sans-serif'}}>
      ...
      <p style={{fontFamily:'Cormorant Garamond,serif',fontSize:15,color:'#EDE6D6',lineHeight:1.7,marginBottom:16}}>Your digital edition will be sent to {form.buyerEmail} shortly.</p>
      ...
      <p style={{fontFamily:'Cormorant Garamond,serif',fontSize:13,color:'#C9A84C',fontStyle:'italic',lineHeight:1.7}}>In that day you will know that I am in the Father, and you in Me, and I in you. John 14:20</p>
      {memberReferralCode && <div style={{marginTop:16,background:'rgba(46,204,113,0.06)',border:'1px solid rgba(46,204,113,0.2)',borderRadius:4,padding:'12px 16px'}}>
  ...
  return (
    <div style={{background:'#12121A',border:'1px solid rgba(201,168,76,0.25)',borderRadius:8,overflow:'hidden',fontFamily:'Inter,sans-serif'}}>
      <div style={{background:'linear-gradient(135deg,#0A0A0F,#1A1A26)',padding:'28px 24px',textAlign:'center' as const,borderBottom:'1px solid rgba(201,168,76,0.15)'}}>
        <p style={{fontSize:9,letterSpacing:3,textTransform:'uppercase' as const,color:'#C9A84C',marginBottom:6}}>PRE-PUBLICATION</p>
        <h2 style={{fontFamily:'Playfair Display,serif',fontSize:36,fontWeight:900,background:'linear-gradient(135deg,#E8C97A,#C9A84C)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text',lineHeight:1,marginBottom:6}}>FOURNITY</h2>
        <p style={{fontFamily:'Cormorant Garamond,serif',fontStyle:'italic',fontSize:14,color:'#E8C97A',marginBottom:8}}>Trinity and I Are Four-nity</p>
        <p style={{fontSize:10,letterSpacing:1,textTransform:'uppercase' as const,color:'#C9A84C'}}>Rev Mokoro Manana</p>
      </div>
      <div style={{padding:'20px 20px 24px'}}>
        <p style={{fontFamily:'Cormorant Garamond,serif',fontSize:15,color:'#EDE6D6',lineHeight:1.7,marginBottom:14}}>A 40-chapter illumination of your God-given identity in Christ — before the foundation of the world.</p>
        <div style={{background:'rgba(201,168,76,0.06)',border:'1px solid rgba(201,168,76,0.15)',borderRadius:4,padding:'12px 14px',marginBottom:14}}>
          ...
          {['Digital edition with Audio Reader — sent immediately','Signed physical copy — after printing','Interactive FOURNITY Workbook','Exclusive surprise gift','Your name as a Founding Reader'].map((item,i)=>(
        ...
        <div style={{marginBottom:14}}>
          {isLaunch ? <>
            ...
            <p style={{fontSize:11,color:'#8A8A9A'}}><strong style={{color:'#E74C3C'}}>{remaining} copies</strong> remaining at launch price</p>
          </> : <span style={{fontFamily:'Playfair Display,serif',fontSize:28,fontWeight:900,color:'#fff'}}>R{price}</span>}
        ...
        {!showForm
          ? <button onClick={()=>setShowForm(true)} style={{width:'100%',background:'linear-gradient(135deg,#C9A84C,#8B6914)',color:'#0A0A0F',border:'none',padding:16,fontSize:14,fontWeight:700,letterSpacing:1,textTransform:'uppercase' as const,cursor:'pointer',borderRadius:3,marginBottom:8,fontFamily:'Inter,sans-serif'}}>Pre-Order Now — R{price}</button>
          : <div>
              ...
              {[['Full Name','buyerName','Your full name','text'],['Email','buyerEmail','your@email.com','email'],['WhatsApp','buyerWhatsapp','+27 83 123 4567','tel'],['Street Address','deliveryAddress','Street address','text'],['Suburb','deliverySuburb','Suburb','text'],['City','deliveryCity','City','text'],['Postal Code','deliveryPostalCode','0001','text'],['Province','deliveryProvince','e.g. Gauteng','text']].map(([label,key,placeholder,type])=>(
                <div key={key} style={{marginBottom:8}}>
                  ...
                  <input type={type} placeholder={placeholder} value={form[key as keyof typeof form]} onChange={e=>setForm(f=>({...f,[key]:e.target.value}))} style={{width:'100%',background:'#1A1A26',border:'1px solid rgba(201,168,76,0.2)',borderRadius:3,padding:'10px 12px',fontSize:13,color:'#F5F0E8',fontFamily:'Inter,sans-serif',outline:'none',boxSizing:'border-box' as const}}/>
                </div>
              ))}
              <button onClick={pay} disabled={processing} style={{width:'100%',background:'linear-gradient(135deg,#C9A84C,#8B6914)',color:'#0A0A0F',border:'none',padding:16,fontSize:14,fontWeight:700,cursor:'pointer',borderRadius:3,marginBottom:8,opacity:processing?0.7:1}}>{processing?'Processing...':'Pay R'+price}</button>
              <button onClick={()=>setShowForm(false)} style={{width:'100%',background:'transparent',color:'#8A8A9A',border:'1px solid rgba(255,255,255,0.1)',padding:12,fontSize:12,cursor:'pointer',borderRadius:3,marginBottom:10}}>Cancel</button>
            </div>
        }
        <p style={{fontSize:11,color:'#8A8A9A',textAlign:'center' as const}}>Secure payment powered by Yoco</p>
      </div>
    </div>
  )
}
