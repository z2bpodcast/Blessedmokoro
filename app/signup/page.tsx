  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signUpError?.message?.includes('already registered')) {
        ...
        setTimeout(() => router.push('/pricing'), 1400)
        return
      }
      ...
      <div style={{ padding: '32px 32px 0', textAlign: 'center' }}>
        ...
        <Link href="/" style={{ display: 'inline-block', marginBottom: '12px' }}>
          <img
            src="/logo.jpg"
            alt="Z2B Logo"
            style={{ height: '64px', width: '64px', borderRadius: '12px', border: '3px solid rgba(212,175,55,0.6)', boxShadow: '0 4px 16px rgba(212,175,55,0.2)', margin: '0 auto', display: 'block' }}
          />
        </Link>
