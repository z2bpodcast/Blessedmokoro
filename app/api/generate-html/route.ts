  ...
  <div class="content-wrap">
    <div class="audio-panel">
      ...
      <div style="display:flex;flex-direction:column;gap:8px;">
        ...
        <div id="chapter-text-${i}" style="display:none;" data-plain="${raw.replace(/[&<>"]/g,' ').replace(/\s+/g,' ').trim()}">${raw.replace(/"/g,'&quot;')}</div>`
        }).join('')}
      </div>
    </div>
  </div>
...
function speakChapter(idx){if(idx<0||idx>=totalChapters)return;synth.cancel();selectChapter(idx);var tx=document.getElementById("chapter-text-"+idx);var tl=document.querySelector("#chapter-item-"+idx+" .ch-title");if(!tx)return;var rawT=tx.getAttribute("data-plain")||tx.textContent||"";var cleanText=rawT.replace(/&quot;/g,"''").replace(/&amp;/g,"and").replace(/&lt;/g," ").replace(/&gt;/g," ").replace(/&[a-z]+;/gi," ").replace(/[‘’]/g,"''").replace(/[“”]/g,"''").replace(/[^a-zA-Z0-9s.,!?'"-:;()]/g," ").replace(/[<][^>]*[>]/g," ").replace(/&quot;/g,"'").replace(/&amp;/g,"and").replace(/&lt;/g,"less than").replace(/&gt;/g,"greater than").replace(/\s+/g," ").trim();utterance=new SpeechSynthesisUtterance((tl?"Chapter "+(idx+1)+". "+tl.innerText+". ":"")+cleanText);utterance.rate=currentSpeed;utterance.pitch=1;utterance.lang="en-ZA";var v=getVoice();if(v)utterance.voice=v;utterance.onend=function(){var s=document.getElementById("chapter-status-"+idx);if(s)s.innerText="✓";var b=document.getElementById("btn-play");if(b)b.innerText="▶ Play";if(idx+1<totalChapters){speakChapter(idx+1);}else{var bp=document.getElementById("btn-play");if(bp)bp.innerText="▶ Play";}};synth.speak(utterance);var b=document.getElementById("btn-play");if(b)b.innerText="⏸ Pause";var s=document.getElementById("chapter-status-"+idx);if(s)s.innerText="🔊";}
