        ...
        <div className="max-w-2xl mx-auto">
          ...
          {referralCode && (
            <div className="mt-4 inline-block bg-[#9b59b6]/20 border border-[#9b59b6]/40 text-[#c084fc] text-xs font-bold px-4 py-2 rounded-full">
              🎁 Referred by a Z2B Pathfinder — you are in the right place
            </div>
          )}
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        {isMember && (
          <div className="bg-[#0d1628] border border-[#D4AF37]/30 rounded-xl p-4 text-center mb-6">
            ...
            <div className="text-xs text-[#7a9cc6]">You are seeing your exclusive Pathfinder rates — up to 68% below Pioneer pricing.</div>
          </div>
        )}
        ...
        {!isMember && (
          <div className="bg-[#111827] border border-[#1a3a6e] rounded-xl p-4 text-center mb-6">
            ...
            <div className="text-xs text-[#7a9cc6] mb-3">
              Z2B members get up to 68% off.{' '}
              <button onClick={() => router.push('/signup')} className="text-[#D4AF37] underline font-bold">Join free as a FAM →</button>
            </div>
            <div className="text-xs text-[#4a5a7e]">FAM members also earn 20% commission on every Amavulandlela referral.</div>
          </div>
        )}
        ...
        <div className="space-y-4">
          {PLANS.map(p => {
            ...
            return (
              <div
                key={p.plan}
                className="rounded-2xl border-2 transition-all cursor-pointer bg-[#111827]"
              ...
              >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {p.popular && (
                            <span className="text-xs font-bold px-2 py-0.5 rounded-full text-black" style={{ background: p.color }}>POPULAR</span>
                          )}
                        </div>
                ...
                {isSelected && (
                  <div className="px-5 pb-5 border-t border-[#1a3a6e] pt-4 space-y-3">
                    ...
                    <button
                      onClick={e => { e.stopPropagation(); initiateBilling(p.plan) }}
                      ...
                      style={{ background: initiating ? '#1a1a2e' : p.color }}
                    >
        ...
        <div className="mt-8 bg-[#0d1f0d] border border-green-900/40 rounded-xl p-5">
          ...
          <p className="text-xs text-[#7a9cc6] leading-relaxed mb-3">
            Share Amavulandlela with your network. Every subscription earns you 20% every month for as long as they stay subscribed.
          </p>
          <p className="text-xs text-[#4a5a7e]">FAM members, Starter, Bronze+ — everyone can refer.</p>
        </div>
      </div>
...
export default function MarketplaceAppsPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
        <div className="text-[#D4AF37] animate-pulse font-bold">Loading Amavulandlela...</div>
      </div>
    }>
      <MarketplaceAppsInner />
    </Suspense>
  )
}
