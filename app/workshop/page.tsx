...
function WorkshopInner() {
  ...
  const handleComplete = async () => {
    if (!scrolledToBottom || score == null || currentSection == null) return; // activityTicked optional
...
What you are about to read is not theory. It is a mirror. It describes the life of millions of employed South Africans who work hard, pay their bills, and still quietly wonder if this is all there is. Read slowly. Be honest with yourself. Nothing here is meant to shame you — everything here is meant to free you.`)
  ...
  // RENDER VIEWS
  // ============================================================
  // ProspectRegistrationGate REMOVED — registration is now handled by
  // WorkshopEmailGate (unified single form). No double registration.
  ...
  if (view === "workshop") return (
    <div style={S.page}>
      {/* ── EC Poster Studio — floating, always visible ── */}
      <ECPosterStudio
        builderRef={builderRef}
        builderName={manlawMemberName}
      />

      {/* ── Purple Cow Share Tool — floating, always visible ── */}
      <PurpleCowShareTool
        builderRef={builderRef}
        builderName={manlawMemberName}
      />
  ...
  if ((view as any) === "morning" && morningSession) return (
    <div style={S.page}>
      {/* ── EC Poster Studio — floating button, always available ── */}
      <ECPosterStudio builderRef={builderRef} builderName={manlawMemberName} />
      ...
      <div style={{ maxWidth: "760px", margin: "0 auto", padding: "16px 20px 40px", display: "flex", justifyContent: "space-between", gap: "12px", flexWrap: "wrap" as const }}>
        ...
        {morningSession.id < 9 && (
          <button
            ...
            onClick={async () => {
              // Mark this morning session as read in workshop_progress
              if (userId) {
                const msId = 200 + morningSession.id; // morning sessions use IDs 201-209
                try {
                  ...
                  recordProgress(msId);
                } catch(e) { console.error("Morning progress save error:", e); }
              }
              openMorningSession(morningSession.id + 1);
            }}>
            Morning {morningSession.id + 1} →
          </button>
        )}
        {morningSession.id === 9 && (
          <button
            ...
            onClick={async () => {
              if (userId) {
                const msId = 200 + morningSession.id;
                try {
                  await supabase.from("workshop_progress").upsert({
                    user_id:      userId,
                    section_id:   msId,
