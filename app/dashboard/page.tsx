// v2026-05-08 — FAM paywall + quick nav links
'use client'
...
import {
  LogOut, Crown, Copy, CheckCircle, Bell, X, Phone
} from 'lucide-react'
...
function DashboardInner() {
    ...
    try {
      ...
      if (!prof) {
        ...
        const { data: newProf } = await supabase.from('profiles').upsert({
          ...
          user_role: 'fam', paid_tier: 'fam', referral_code: refCode,
          is_paid_member: false, payment_status: 'free',
      ...
      setProfileComplete(!(!prof.whatsapp_number || !prof.city))
      ...
      const { data: earningsData } = await supabase
        .from('comp_earnings').select('amount, created_at, status')
        .eq('builder_id', user.id).in('status', ['confirmed', 'paid'])
  ...
  const loadNotifications = async (userId: string) => {
    const { data } = await supabase.from('prospect_notifications').select('*')
      .eq('builder_id', userId).order('created_at', { ascending: false })
    if (data) {
  ...
  const markAllRead = async () => {
    if (!user) return
    await supabase.from('prospect_notifications').update({ read: true })
      .eq('builder_id', user.id).eq('read', false)
  ...
  const copy4MLink = () => {
    if (!profile) return
    navigator.clipboard.writeText(window.location.origin + '/4m?ref=' + profile.referral_code)
  ...
  const saveProfile = async () => {
    ...
    await supabase.from('profiles').update({
      whatsapp_number: profileForm.whatsapp, city: profileForm.city,
      province: profileForm.province, occupation: profileForm.occupation,
    }).eq('id', user.id)
    setProfile(prev => prev ? { ...prev, whatsapp_number: profileForm.whatsapp, city: profileForm.city } : prev)
    setProfileComplete(true); setShowProfileModal(false); setSavingProfile(false)
  }
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="bg-green-50 border-b-4 border-green-400 px-4 py-4">
          <div className="max-w-7xl mx-auto flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-green-700 text-sm">Your membership has been upgraded. Enjoy your new benefits!</p>
            </div>
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="absolute rounded-full opacity-20"
              style={{
                width:`${[80,60,100,50,70,90][i]}px`, height:`${[80,60,100,50,70,90][i]}px`,
                background:`radial-gradient(circle,${['#DC2626','#9333EA','#DC2626','#7C3AED','#B91C1C','#6D28D9'][i]},transparent)`,
                left:`${[5,20,45,65,80,92][i]}%`, top:`${[10,60,20,70,30,50][i]}%`,
        ...
        <div className="max-w-7xl mx-auto flex items-center gap-4 flex-wrap relative z-10">
          <div className="flex items-center gap-3 flex-1">
            <div className="relative flex-shrink-0">
              <div className="absolute inset-0 rounded-full animate-ping" style={{ background:'rgba(220,38,38,0.3)', animationDuration:'2s' }}/>
              <span className="relative text-2xl">❤️</span>
            </div>
            <div>
              <p className="font-black text-white text-sm tracking-wide">Welcome to Abundance</p>
              <p className="text-xs mt-0.5" style={{ color:'rgba(255,255,255,0.45)' }}>Your orientation guide — always here when you need direction</p>
            </div>
          </div>
          <a href="/start-here"
            ...
            style={{ background:'linear-gradient(135deg,rgba(220,38,38,0.2),rgba(185,28,28,0.1))', borderColor:'rgba(220,38,38,0.4)', color:'#FCA5A5' }}>
            <span className="animate-pulse">❤️</span> Start Here →
          </a>
        </div>
      </div>

      {/* Floating Coach Manlaw */}
      {showStartHere && (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-4 rounded-2xl shadow-2xl font-black text-white text-sm border-2 border-yellow-400"
          ...
          <button onClick={() => { setShowStartHere(false); try { localStorage.setItem('z2b_start_here_dismissed','1') } catch(e) {} }}
            className="ml-2 flex items-center justify-center w-6 h-6 rounded-full text-xs font-black hover:bg-white/20"
            style={{ background:'rgba(255,255,255,0.1)', color:'rgba(255,255,255,0.7)', border:'1px solid rgba(255,255,255,0.2)', flexShrink:0 }}>×</button>
        </div>
      )}
        ...
        <div className="border-b-4 border-amber-400 px-4 py-4 animate-pulse"
          ...
          <div className="max-w-7xl mx-auto flex items-center gap-4">
            ...
            <div className="flex-1">
              <p className="font-black text-amber-800">{(pendingUpgrade || profile?.paid_tier || 'Tier').toUpperCase()} upgrade pending verification</p>
              <p className="text-amber-700 text-sm">Your EFT / ATM payment has been recorded. We will activate within 24 hours.</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {isAdmin && (
          <div className="mb-8 rounded-2xl p-6 border-4 border-yellow-400 shadow-2xl"
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h2 className="text-2xl font-black text-white">{roleInfo ? `${roleInfo.emoji} ${roleInfo.label} Panel` : 'Admin Panel'}</h2>
                <p className="text-purple-300 text-sm">System access granted</p>
              </div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
              <a href="/z2b-command-7x9k/hub" className="bg-yellow-400 text-purple-900 font-black py-3 px-4 rounded-xl text-center text-sm hover:bg-yellow-300 transition-all border-2 border-yellow-300">🏛️ Admin Hub</a>
              {isSuperAdmin && (<>
                <a href="/z2b-command-7x9k/members" className="bg-white/10 border border-white/30 text-white font-bold py-3 px-4 rounded-xl text-center text-sm hover:bg-white/20">👥 Members</a>
                <a href="/z2b-command-7x9k/payments" className="bg-white/10 border border-white/30 text-white font-bold py-3 px-4 rounded-xl text-center text-sm hover:bg-white/20">💳 Payments</a>
                <a href="/z2b-command-7x9k/compensation" className="bg-white/10 border border-white/30 text-white font-bold py-3 px-4 rounded-xl text-center text-sm hover:bg-white/20">💎 Compensation</a>
                <a href="/z2b-command-7x9k/earnings" className="bg-white/10 border border-white/30 text-white font-bold py-3 px-4 rounded-xl text-center text-sm hover:bg-white/20">📊 Earnings</a>
              </>)}
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border-2 border-purple-200 shadow-sm p-6 mb-6">
          <div className="flex items-start justify-between flex-wrap gap-4 mb-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                {profile.referred_by && (
                  <p className="text-xs text-purple-500 mt-1">Sponsor: <strong>{profile.sponsor_name || profile.referred_by}</strong></p>
                )}
              </div>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Stats row */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t-2 border-gray-100 mb-4">
            <div>
          ...
          <div className="flex gap-2 flex-wrap pt-4 border-t-2 border-gray-100">
            ...
            <a href="/marketplace"
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-black border-2 hover:scale-105 transition-transform"
              style={{ background:'linear-gradient(135deg,#042030,#063048)', borderColor:'#06B6D4', color:'#67E8F9' }}>
              🏪 Marketplace
            </a>
            <a href="/earn"
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-black border-2 hover:scale-105 transition-transform"
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-600 text-sm mt-0.5">
                ...
                {isFam     ? 'upgrade to Starter Pack (R500) to unlock the full Z2B Book System + 4M Machine' :
                 ...
                 tier==='gold'    ? 'upgrade to Platinum (R50,000) for 30% ISP + all 9 streams + Distribution License' : ''}
              </p>
            </div>
            <Link href="/pricing"
        ...
        <div className="bg-white rounded-2xl border-2 border-yellow-300 shadow-sm p-6 mb-6">
          ...
          <p className="text-xs text-gray-500 mb-4">Two powerful links — share the right one for the right audience</p>
          <div className="bg-purple-50 border-2 border-purple-200 rounded-xl p-4 mb-3">
            <div className="flex items-center gap-2 mb-1"><span className="text-base">🍽️</span><span className="text-xs font-black text-purple-700 uppercase tracking-wider">Z2B Table Banquet Link</span></div>
          ...
          <div className="bg-yellow-50 border-2 border-yellow-300 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-1"><span className="text-base">🤖</span><span className="text-xs font-black text-yellow-700 uppercase tracking-wider">Z2B 4M Income Executor Link</span></div>
            <p className="text-xs text-gray-500 mb-2">For beginners who need to start earning first</p>
        ...
        <div className="bg-white rounded-2xl border-2 border-gray-200 shadow-sm p-6">
          <div style={{ marginBottom:'16px' }}><WorkshopProgressBar /></div>
          <h3 className="text-lg font-black text-gray-800 mb-4">Quick Actions</h3>

          {/* ── FAM SOFT PAYWALL ── */}
          {isFam && (
            <div className="rounded-2xl p-5 mb-4 text-center border-2 border-yellow-400"
              style={{ background:'linear-gradient(135deg,#1e1b4b,#4c1d95)' }}>
              ...
              <p className="text-purple-200 text-sm mb-4 leading-relaxed">
                ...
                As a FAM member you can share links and earn 20% commission on every sale.
              </p>
              <div className="flex gap-3 justify-center flex-wrap mb-4">
                <a href="/marketplace/product/book-ecosystem"
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-black border-2 border-[#c9a227] hover:scale-105 transition-transform"
                  style={{ background:'linear-gradient(135deg,#1a0d35,#2d1b69)', color:'#f0c040' }}>
                  📖 Share Z2B Book — Earn 20%
                </a>
                <a href="/earn"
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-black border-2 border-white/30 text-white hover:bg-white/10 transition-all">
                  💰 See All Products to Share
                </a>
              </div>
              <Link href="/pricing"
          ...
          <Link href="/meet-coach-manlaw"
            className={`flex items-center gap-3 p-4 rounded-xl border-2 font-bold text-sm hover:scale-105 transition-transform mb-4 ${isFam ? 'opacity-30 pointer-events-none select-none' : ''}`}
          ...
          {/* 6 core quick actions — greyed out for FAM */}
          <div className={`grid grid-cols-3 gap-3 mb-4 ${isFam ? 'opacity-25 pointer-events-none select-none' : ''}`}>
            <Link href="/workshop"
              ...
              <span className="text-2xl">🎓</span><span>Workshop</span>
              <span className="text-xs font-normal" style={{ color:'rgba(120,53,15,0.65)' }}>99 sessions</span>
            </Link>
            <Link href="/invite"
              className="flex flex-col items-center gap-2 p-4 rounded-xl border-2 font-bold text-center text-sm hover:scale-105 transition-transform"
              style={{ background:'linear-gradient(135deg,#1C0500,#7C2D12)', borderColor:'#FB923C', color:'#FED7AA' }}>
              <span className="text-2xl">🎴</span><span>Invite</span>
            ...
            <Link href="/my-earnings"
              ...
              <span className="text-2xl">💰</span><span>Earnings</span>
            ...
            <Link href="/content-studio-plus"
              ...
              <span className="text-2xl">📱</span><span>Content Engine</span>
            ...
            <Link href="/my-funnel"
              ...
              <span className="text-2xl">🎯</span><span>My Funnel</span>
            ...
            <Link href="/daily-spark"
              ...
              <span className="text-2xl">⚡</span><span>Daily Spark</span>
          ...
          {showAllFeatures && (
            <>
              <div onClick={() => setShowAllFeatures(false)}
                style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:40, backdropFilter:'blur(4px)' }}/>
              <div style={{ position:'fixed', bottom:0, left:0, right:0, zIndex:50, background:'linear-gradient(180deg,#0D0A1E,#09060F)', border:'1.5px solid rgba(212,175,55,0.25)', borderRadius:'24px 24px 0 0', padding:'24px 20px 40px', maxHeight:'82vh', overflowY:'auto' }}>
                <div style={{ width:'40px', height:'4px', background:'rgba(255,255,255,0.15)', borderRadius:'2px', margin:'0 auto 20px' }}/>
                <div style={{ fontFamily:'Cinzel,serif', fontSize:'11px', letterSpacing:'3px', color:'rgba(212,175,55,0.5)', marginBottom:'18px', textAlign:'center' }}>ALL FEATURES</div>
                ...
                <div className="grid grid-cols-3 gap-3 mb-5">
                  {[
                    ...
                    <Link key={item.href} href={item.href} onClick={() => setShowAllFeatures(false)}
          ...
          {/* All Features button — disabled for FAM */}
          <button
            onClick={() => !isFam && setShowAllFeatures(true)}
            disabled={isFam}
            style={{
              width:'100%', padding:'14px', marginTop:'4px',
              background: isFam ? 'rgba(107,114,128,0.05)' : 'rgba(212,175,55,0.07)',
              border:`1.5px solid ${isFam ? 'rgba(107,114,128,0.15)' : 'rgba(212,175,55,0.25)'}`,
              borderRadius:'14px', cursor: isFam ? 'not-allowed' : 'pointer',
              display:'flex', alignItems:'center', justifyContent:'center', gap:'10px',
              fontFamily:'Cinzel,serif', fontSize:'13px', fontWeight:700,
              color: isFam ? 'rgba(107,114,128,0.4)' : 'rgba(212,175,55,0.8)',
              letterSpacing:'1px',
            }}>
            <span style={{ fontSize:'18px' }}>{isFam ? '🔒' : '☰'}</span>
            {isFam ? 'All Features — Upgrade to Unlock' : 'All Features'}
            <span style={{ fontSize:'12px', opacity:0.5 }}>{isFam ? '' : '↑'}</span>
          </button>
        </div>
      </div>
      ...
      {showNotifications && (
        <div className="fixed inset-0 z-50 flex justify-end" style={{ background:'rgba(0,0,0,0.55)' }}
          ...
          <div className="w-full max-w-md h-full bg-white shadow-2xl flex flex-col"
            style={{ animation:'slideInRight 0.3s ease-out' }} onClick={e => e.stopPropagation()}>
            <div style={{ background:'linear-gradient(135deg,#6B21A8,#9333EA)' }}
              ...
              <button onClick={() => setShowNotifications(false)} className="bg-white/20 rounded-lg p-2 text-white hover:bg-white/30">
                <X className="w-5 h-5"/>
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-4">
              {notifications.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <p className="text-gray-500 text-sm leading-relaxed">No prospect notifications yet.<br/>Share your workshop link and watch them appear here!</p>
                </div>
              ) : (
                notifications.map(n => (
                  <div key={n.id} className={`rounded-2xl p-4 mb-3 border-2 relative ${n.read ? 'bg-gray-50 border-gray-200' : 'border-purple-300'}`}
                    style={!n.read ? { background:'linear-gradient(135deg,#F3E8FF,#EDE9FE)' } : {}}>
                    {!n.read && <div className="absolute top-3 right-3 w-2.5 h-2.5 bg-purple-600 rounded-full"/>}
                    ...
                    <div className="flex items-center gap-2 bg-white border border-purple-200 rounded-xl p-3 mb-3">
                      ...
                      <a href={`https://wa.me/${n.prospect_whatsapp.replace(/\D/g,'')}?text=${encodeURIComponent(`Hi ${n.prospect_name}! I'm your Z2B Builder. I saw you completed Section ${n.section_id} 🎉 Let's talk about your next steps!`)}`}
                        target="_blank" rel="noopener noreferrer" className="bg-green-500 text-white text-xs font-bold px-3 py-1.5 rounded-lg">WhatsApp</a>
                    </div>
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <div className="flex gap-2">
                        {['contacted','dismissed'].map(s => (
                          <button key={s} onClick={() => updateStatus(n.id, s)}
                            className={`text-xs px-3 py-1.5 rounded-lg font-bold border ${n.status===s ? s==='contacted' ? 'bg-green-100 text-green-700 border-green-300' : 'bg-red-100 text-red-600 border-red-300' : 'bg-gray-100 text-gray-600 border-gray-200'}`}>
                            {s==='contacted' ? '✅ Contacted' : '❌ Dismiss'}
                          </button>
                        ))}
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-gray-400">{new Date(n.created_at).toLocaleDateString('en-ZA',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'})}</span>
                        <button onClick={() => deleteNotification(n.id)} className="text-gray-300 hover:text-gray-500"><X className="w-3.5 h-3.5"/></button>
                      </div>
                    </div>
                  </div>
                ))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background:'rgba(0,0,0,0.6)' }}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              {[
                { key:'whatsapp', label:'WhatsApp Number *', type:'tel', ph:'+27 81 234 5678' },
                { key:'city',     label:'City *',            type:'text', ph:'e.g. Pretoria' },
                { key:'province', label:'Province',          type:'text', ph:'e.g. Gauteng' },
                { key:'occupation',label:'Occupation',       type:'text', ph:'e.g. Teacher, Nurse, Entrepreneur' },
              ].map(f => (
                <div key={f.key}>
                  <label className="text-xs font-black text-gray-600 mb-1 block">{f.label}</label>
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setShowProfileModal(false)} className="flex-1 bg-gray-100 text-gray-600 font-bold py-3 rounded-xl text-sm">Later</button>
