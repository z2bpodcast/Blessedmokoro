...
const DEFAULT_RULES = {
  isp: {
    starter: 10, bronze: 18, copper: 22, silver: 25, gold: 28, platinum: 30
  },
  qpb: {
    starter_qualifies: false,
    min_sales: 4,
    first_set_rate: 7.5,
    subsequent_rate: 10,
  ...
  tsc: {
    ...
    max_gen: { starter: 0, bronze: 3, copper: 4, silver: 6, gold: 8, platinum: 10 },
  },
  mkt: {
    builder_share: 95,
    platform_fee: 5,
...
const TIER_COLORS: Record<string, string> = {
  starter: '#6B7280', bronze: '#CD7F32', copper: '#B87333',
  silver: '#9CA3AF', gold: '#D4AF37', platinum: '#9333EA'
}
const TIER_BG: Record<string, string> = {
  starter: '#F9FAFB', bronze: '#FEF3C7', copper: '#FFF7ED',
  silver: '#F3F4F6', gold: '#FEFCE8', platinum: '#F3E8FF'
}
const TIERS = ['starter', 'bronze', 'copper', 'silver', 'gold', 'platinum']
...
const BFM_RATES: Record<string,{base:number,pct:number,monthly:number}> = {
  starter:  { base:800, pct:0.10, monthly:850 },
  bronze:   { base:800, pct:0.10, monthly:1050 },
  copper:   { base:800, pct:0.10, monthly:1300 },
  silver:   { base:800, pct:0.10, monthly:2000 },
  gold:     { base:800, pct:0.10, monthly:3200 },
  platinum: { base:800, pct:0.10, monthly:5800 },
}

const TIER_PRICES_ONCE: Record<string,number> = {
  ...
  const loadData = async () => {
    ...
    if (earningsData) {
      const enriched = earningsData.map((e: any) => ({
        ...
        paid_tier:      e.profiles?.paid_tier || e.tier_at_earning || 'starter',
                      ...
                      <tr key={e.id} style={{ background: i%2===0 ? '#0f0a1e' : '#1e1b4b' }}>
                        ...
                        <td className="px-4 py-3">
                          <span className="text-xs font-black px-2 py-1 rounded-full"
                            ...
                            {(e.paid_tier||'starter').toUpperCase()}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-xs font-bold px-2 py-1 rounded-full"
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
                {TIERS.map(t => (
                  <div key={t} className="rounded-xl p-3 text-center border"
                    style={{ background: t === 'starter' ? '#FEE2E2' : TIER_BG[t], borderColor: t === 'starter' ? '#FCA5A5' : TIER_COLORS[t]+'60' }}>
                    <div className="text-xs font-black mb-1" style={{ color: t === 'starter' ? '#DC2626' : TIER_COLORS[t] }}>
                      {t.toUpperCase()}
                    </div>
                    <div className="text-sm font-black" style={{ color: t === 'starter' ? '#DC2626' : TIER_COLORS[t] }}>
                      {t === 'starter' ? '❌ No QPB' : '✅ Qualifies'}
                    </div>
                  </div>
                ))}
              </div>
